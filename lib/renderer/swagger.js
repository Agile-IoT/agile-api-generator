...
SwaggerRenderer.prototype.processTypes = function (defTypes, depth) {
  ...
  _.forEach(defTypes, function (type, typeName) {
    ...
    var baseObjType = type.type.replace('*', '');
    var typeReference = me.definition.types[baseObjType];
    var objtype = baseObjType.toLowerCase();
    ...
    if(type.isArray()) {
      ...
      if(swaggerType.type !== undefined)
        delete swaggerType.type;
      swaggerType.$ref = '#/definitions/' + typeReference.name;
      me.addDef(typeReference.name);
    } else {
      swaggerType.type = objtype ? objtype.toLowerCase() : "object";
...
SwaggerRenderer.prototype.processEndpoint = function (path, endpoint, api) {
  ...
  _.each(endpoint, function(def, method) {
    ...
    if(def.body && !hasBody) {
      ...
      var templatedMethod = "{{url}}" + method;
      endpoint[templatedMethod] = endpoint[method];
      delete endpoint[method];
      endpoint[templatedMethod].parameters = parameters;
    }
